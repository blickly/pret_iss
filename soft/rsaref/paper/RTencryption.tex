...
\begin{SCfigure}
  ...
  \caption{By enclosing the add instruction with deadlines, we ensure that the 2 add's are executed 8 cycles apart. \$t0 a deadline register. When a deadline instruction is encountered, the program does not progress forward until the deadline register is decremented to 0.}
  \label{fig:deadline}
\end{SCfigure}
