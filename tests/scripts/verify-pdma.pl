...
sub get_pret_include
{
    if( ! defined $ENV{ 'PRET_ISS' } ) {
        &print_log( "Error: " .
                   "environment variable PRET_ISS is not defined!\n" );
        exit 1;
    }

    my $pret_home = $ENV{ 'PRET_ISS' };
    $pret_home=~s(\\)(/)g;  #replace any backslash with forward slash
    $pret_home=$pret_home . "/include ";

    if( ! defined $ENV{ 'CXXFLAGS' } ) {
        &print_log( "Error: " .
                   "environment variable CXXFLAGS is not defined!\n" );
        exit 1;
    }

    $pret_home=$pret_home . $ENV{ 'CXXFLAGS'}
...
sub init_globals
{
    ...
    $pret_include = "-I".&get_pret_include;
...
#        $rt_add_ldlibs = $ENV{ 'RT_ADD_LDLIBS' };
        $rt_add_ldlibs = $ENV{'PRET_ISS'} . '/src/pdma/libpretpdma.a -pthread'	
    }
    ...
    $rt_strip_tail_pat = '^(SystemC:|End of SystemC Simulation)';
...
sub strip_more_tail
{
    ...
    local( \$tmp ) = $#strip_logfile;
    while( \$tmp >= 0 ) {
        last if \$strip_logfile[\$tmp] =~ /^SystemC:/;
        \$tmp --;
    }
    \$#strip_logfile = \$tmp - 1 unless \$tmp < 0;
EVAL
